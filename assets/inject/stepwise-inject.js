(() => {
  ...
  function installStyle() {
    ...
    style.textContent = `
      ...
      @container csw-panel (max-width: 360px) {
        .csw-click-mode .csw-metric-label {
          display: none;
        }
      }
  ...
  function resizeAnchorForWidth(nextWidth, nextLeft, corner) {
    const chipWidth = state.layout?.chip?.width || CHIP_WIDTH;
    if (corner === "bl") return nextLeft + (nextWidth - chipWidth) / 2;
    return nextLeft + (nextWidth - chipWidth) / 2;
  }
  ...
  function installResize() {
    ...
    state.popover.querySelectorAll(".csw-resize-handle").forEach((handle) => {
      handle.addEventListener("pointerdown", (event) => {
        ...
        const resize = { pointerId: event.pointerId, corner };
        ...
        const onMove = (moveEvent) => {
          ...
          const nextWidth = clampPanelWidth(corner === "bl" ? startWidth - dx : startWidth + dx);
          const nextLeft = corner === "bl"
            ? startRight - nextWidth
            : startLeft;
          const nextHeight = state.layout.opensDown
            ? clampPanelHeight(startHeight + dy)
            : clampPanelHeight(startHeight - dy);
          state.width = nextWidth;
          state.height = nextHeight;
          state.position = clampPosition({
            x: resizeAnchorForWidth(nextWidth, nextLeft, corner),
            y: state.position?.y,
          });
          applyPosition();
        };
  ...
  function outlineHeadingCandidate(node, kind) {
    ...
    if (node.matches("strong,b")) {
      ...
      return { el: node, text: displayText, level: 3, kind };
    }
    ...
    if (score < OUTLINE_PSEUDO_MIN_SCORE) return null;
    const numbered = /^\d{1,2}[\.、．\)]\s*\S{2,}/.test(text)
      || /^[一二三四五六七八九十]+[、.．]\s*\S{2,}/.test(text);
    return { el: node, text: displayText, level: numbered ? 2 : text.length <= 12 ? 2 : 3, kind };
  }
  ...
  function outlineNormalizeDisplayLevels(items) {
    ...
    items.forEach((item) => {
      item.displayLevel = item.level - minimumLevel;
    });
    return items;
  }
  ...
  function outlineJumpTo(id) {
    ...
    state.panel?.querySelectorAll("[data-outline-id]").forEach((button) => {
      const isActive = button.dataset.outlineId === id;
    ...
    window.requestAnimationFrame(() => {
      ...
      if (rect.top < 40 || rect.top > window.innerHeight * 0.55) {
        ...
        if (container && container !== document.documentElement && container !== document.body) {
          ...
          const next = container.scrollTop + rect.top - containerRect.top - 80;
          try { container.scrollTo({ top: next, behavior: "smooth" }); } catch { container.scrollTop = next; }
        }
      }
      window.setTimeout(() => {
        if (element.style.scrollMarginTop === "88px") element.style.scrollMarginTop = previousMargin;
      }, 420);
    });
  ...
  function outlineHtml() {
    ...
    return `<div class="csw-outline-list" role="list">${state.outlineItems.map((item) => {
      const displayLevel = item.displayLevel ?? 0;
      return `
        <button class="csw-outline-row" type="button" role="listitem" data-outline-id="${escapeAttr(item.id)}" data-level="${displayLevel}" style="--csw-outline-indent:${Math.min(3, displayLevel) * 12}px">
          <span class="csw-outline-text">${escapeHtml(item.text)}</span>
        </button>
      `;
    }).join("")}</div>`;
  }

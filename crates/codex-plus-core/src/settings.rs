...
fn aggregate_model_intersection(profiles: &[&RelayProfile]) -> Option<AggregateModelIntersection> {
    ...
    let mut common = first_models.iter().cloned().collect::<HashSet<_>>();
    let mut windows = profile_model_windows(first);

    for profile in profiles {
        let model_set = profile_model_order(profile)
            .into_iter()
            .collect::<HashSet<_>>();
...
experimental_bearer_token = "sk-existing""#
    ...
    #[test]
    fn settings_normalization_removes_invalid_duplicate_aggregate_members_and_stale_id() {
        let dir = temp_dir();
        let store = SettingsStore::new(dir.join("settings.json"));
                ...
                "aggregateRelayProfiles": [
                    {
                        ...
                        "members": [
                            { "relayId": "relay-a", "weight": 1 },
                            { "relayId": "relay-a", "weight": 9 },
                            { "relayId": "missing", "weight": 1 }
                        ]
                    },
                    { "id": "orphan", "name": "Orphan", "members": [] }
                ]
            }))
            .unwrap();

        assert_eq!(updated.active_relay_id, "relay-a");
        assert!(updated.active_aggregate_relay_id.is_empty());
        assert_eq!(updated.aggregate_relay_profiles.len(), 1);
        assert_eq!(updated.aggregate_relay_profiles[0].members.len(), 1);
        assert_eq!(
            updated.aggregate_relay_profiles[0].members[0].relay_id,
            "relay-a"
        );
    }

...
pub fn model_router_profile(settings: &BackendSettings, home: &Path) -> RelayProfile {
    ...
    for profile in settings.relay_profiles.iter().filter(|profile| {
        ...
        {
            let (model, suffix_window) = crate::model_suffix::parse_model_suffix(raw_model);
            if model.is_empty() || !seen_models.insert(model.clone()) {
                continue;
            }

            routed_models.push(model.clone());
            let window = suffix_window
                .map(|value| value.to_string())
                .or_else(|| model_windows.get(&model).cloned());
            if let Some(window) = window {
                routed_windows.insert(model, window);
            }
        }
    }

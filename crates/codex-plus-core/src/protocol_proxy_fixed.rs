...
pub fn responses_to_chat_completions(body: Value) -> anyhow::Result<Value> {
    ...
    // Strip image_url blocks for text-only providers that reject multimodal content
    // e.g. DeepSeek returns: "unknown variant image_url, expected text"
    let model = body.get("model").and_then(Value::as_str).unwrap_or("");
    if !model_supports_vision(model) {
        strip_image_url_from_messages(&mut messages);
    }
...
fn model_supports_vision(model: &str) -> bool {
    // DeepSeek models do not support image_url content blocks.
    // Other text-only providers can be added here when needed.
    !matches!(
...
fn strip_image_url_from_messages(messages: &mut [Value]) {
    for message in messages {
        ...
        if filtered.is_empty() {
            *content = json!("[Image]");
            ...
            && filtered[0].get("type").and_then(Value::as_str) == Some("text")
        {
            *content = filtered[0].get("text").cloned().unwrap_or(json!(""));
        } else {

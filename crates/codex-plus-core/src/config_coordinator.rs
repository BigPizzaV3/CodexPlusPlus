...
pub fn coordination_status(settings: &BackendSettings) -> CoordinationStatus {
    ...
    CoordinationStatus {
        ...
        conflict_detected,
        conflict_message,
        ccswitch_current_provider_id: current_ccs
            .as_ref()
            .map(|provider| provider.source_id.clone()),
...
pub fn evaluate_live_write(settings: &BackendSettings, force: bool) -> LiveConfigWriteDecision {
    if !settings.relay_profiles_enabled {
        return LiveConfigWriteDecision {
            allowed: false,
            message: "供应商配置总开关已关闭，Codex++ 不会写入 config.toml / auth.json。"
                .to_string(),
        };
    }
    ...
    if detect_ccswitch()
        ...
        && !force
        && let Some(conflict) =
            detect_external_modification(&relay_config::default_codex_home_dir())
    {
        return LiveConfigWriteDecision {
            allowed: false,
            message: format!(
...
fn coordination_guidance(
    ...
    if conflict_detected {
        return "检测到配置冲突：请统一只在一个工具中切换供应商，或明确设置“配置所有权”。"
            .to_string();
    }
    match effective {
        ConfigOwnership::CcSwitch => {
        ...
        ConfigOwnership::Auto => {
            "自动模式会根据联动开关和 CC Switch 是否存在选择所有权。".to_string()
        }
    }
}

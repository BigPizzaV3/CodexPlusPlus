...
export type ModelWindowRow = {
  model: string;
  window: string;
  /// 自动压缩百分比（如 "90" 表示 context_window 的 90%）；空字符串表示用 Codex 默认 90%。
  autoCompact: string;
...
export function serializeModelWindowRows(rows: ModelWindowRow[]): { modelList: string; modelWindows: string; modelVlm: string; modelAutoCompact: string } {

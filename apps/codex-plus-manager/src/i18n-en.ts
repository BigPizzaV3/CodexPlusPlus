...
export const EN_PLAIN: Record<string, string> = {
  ...
  "上下文大小": "Context size",
  "留空不写入全局值；填写后作为该供应商下所有模型的全局上下文上限，并受 catalog 中 max_context_window 约束。使用逐模型窗口时建议留空。":
    "Leave blank to avoid writing a global value. When set, it acts as a global context limit for every model from this provider and remains constrained by max_context_window in the catalog. Leave it blank when using per-model windows.",
  ...
  "留空不写入全局值；填写后覆盖该供应商下所有模型的压缩触发 token 数。使用逐模型自动压缩时建议留空。":
    "Leave blank to avoid writing a global value. When set, it overrides the compaction trigger token count for every model from this provider. Leave it blank when using per-model auto-compaction.",
...
export const EN_TEMPLATE: Record<string, string> = {
  ...
  "以下字段由 Codex++ 管理，未导入：{0}": "These fields are managed by Codex++ and were not imported: {0}",
  "作者：{0} · {1}": "Author: {0} · {1}",
...
export const EN_BACKEND: Record<string, string> = {
  ...
  "当前供应商配置已重新应用。": "The current provider configuration was reapplied.",
...
export const EN_BACKEND_PATTERNS: Array<[RegExp, string]> = [
  [/^每模型配置无效：model_windows JSON 解析失败：(.+)$/, "Invalid per-model configuration: failed to parse model_windows JSON: $1"],
  [/^每模型配置无效：model_auto_compact JSON 解析失败：(.+)$/, "Invalid per-model configuration: failed to parse model_auto_compact JSON: $1"],
  [/^每模型配置无效：model_metadata JSON 解析失败：(.+)$/, "Invalid per-model configuration: failed to parse model_metadata JSON: $1"],
  [/^每模型配置无效：(model_windows|model_auto_compact|model_metadata) 必须是 JSON 对象$/, "Invalid per-model configuration: $1 must be a JSON object"],
  [/^每模型配置无效：(model_windows|model_auto_compact) 的模型 (.+?) 值必须是字符串$/, "Invalid per-model configuration: model $2 in $1 must have a string value"],
  [/^每模型配置无效：model_windows 的模型 (.+?) 窗口值无效：(.+)$/, "Invalid per-model configuration: model $1 has an invalid context window: $2"],
  [/^每模型配置无效：model_auto_compact 的模型 (.+?) 百分比无效：(.+)$/, "Invalid per-model configuration: model $1 has an invalid auto-compact percentage: $2"],
  [/^每模型配置无效：model_metadata 的模型 (.+?) 值必须是对象$/, "Invalid per-model configuration: model $1 in model_metadata must have an object value"],
  [/^每模型配置无效：(.+)$/, "Invalid per-model configuration: $1"],
  [/^重新应用当前供应商配置失败：当前配置使用外部 model_catalog_json，无法同时应用每模型窗口、自动压缩或元数据$/, "Failed to reapply the current provider configuration because it uses an external model_catalog_json. Per-model windows, auto-compact settings, and metadata cannot be applied at the same time."],
  [/^重新应用当前供应商配置失败：当前 Codex 配置使用外部 model_catalog_json，无法同时应用每模型窗口、自动压缩或元数据$/, "Failed to reapply the current provider configuration because the current Codex configuration uses an external model_catalog_json. Per-model windows, auto-compact settings, and metadata cannot be applied at the same time."],
  [/^重新应用当前供应商配置失败：catalog JSON 解析失败：(.+)$/, "Failed to reapply the current provider configuration because the catalog JSON could not be parsed: $1"],
  [/^重新应用当前供应商配置失败：(.+)$/, "Failed to reapply the current provider configuration: $1"],
  [/^切换完整中转配置失败：当前配置使用外部 model_catalog_json，无法同时应用每模型窗口、自动压缩或元数据$/, "Failed to switch the full relay configuration because it uses an external model_catalog_json. Per-model windows, auto-compact settings, and metadata cannot be applied at the same time."],
  [/^切换完整中转配置失败：当前 Codex 配置使用外部 model_catalog_json，无法同时应用每模型窗口、自动压缩或元数据$/, "Failed to switch the full relay configuration because the current Codex configuration uses an external model_catalog_json. Per-model windows, auto-compact settings, and metadata cannot be applied at the same time."],
  [/^切换完整中转配置失败：catalog JSON 解析失败：(.+)$/, "Failed to switch the full relay configuration because the catalog JSON could not be parsed: $1"],
  [/^当前配置使用外部 model_catalog_json，无法同时应用每模型窗口、自动压缩或元数据$/, "The current configuration uses an external model_catalog_json, so per-model windows, auto-compact settings, and metadata cannot be applied at the same time."],
  [/^当前 Codex 配置使用外部 model_catalog_json，无法同时应用每模型窗口、自动压缩或元数据$/, "The current Codex configuration uses an external model_catalog_json, so per-model windows, auto-compact settings, and metadata cannot be applied at the same time."],
  [/^catalog JSON 解析失败：(.+)$/, "Failed to parse catalog JSON: $1"],

...
export const EN_PLAIN: Record<string, string> = {
  ...
  "API Key 只写入本机 Codex 配置，不会出现在日志中。":
    "The API Key is written only to your local Codex config and never appears in logs.",
  "① 选择模型": "1. Choose model",
  "② 填写 API Key": "2. Enter API Key",
  "③ 预览与应用": "3. Preview & apply",
  "DeepSeek": "DeepSeek",
  "DeepSeek API Key": "DeepSeek API Key",
  "DeepSeek 接入": "DeepSeek Onboarding",
  "DeepSeek 官方接入": "DeepSeek Official Onboarding",
  "Base URL": "Base URL",
  "Responses API（直连）": "Responses API (direct)",
  "按 DeepSeek 官方 Codex 接入教程生成配置：Responses 直连 + 官方模型元数据。":
    "Generates config per DeepSeek's official Codex guide: native Responses API + official model metadata.",
  "按官方教程一键接入 DeepSeek 模型": "One-click DeepSeek onboarding following the official guide",
  "官方模型元数据已启用": "Official model metadata enabled",
  "官方提示：deepseek-v4-flash 已支持接入 Codex，deepseek-v4-pro 预计 2026 年 8 月初支持。":
    "Official note: deepseek-v4-flash supports Codex now; deepseek-v4-pro is expected in early August 2026.",
  "应用并切换": "Apply & switch",
  "备份路径": "Backup path",
  "默认模型": "Default model",
  "配置路径": "Config path",
  "请先填写 API Key。": "Please enter an API Key first.",
  "重启 Codex / Codex++ 后生效。": "Restart Codex / Codex++ for it to take effect.",

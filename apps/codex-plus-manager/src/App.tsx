...
function RelayProfileDetail({
  ...
  const saveDraft = async () => {
    ...
    const saved = await onFormChange(next);
    if (!saved) return;
    if (isActive && relayProfileUsesLiveFiles(normalizedDraft)) {
      ...
      const authSaved = await actions.saveRelayFile("auth", normalizedDraft.authContents, true);
      if (!authSaved) return;
    }
    onSaved?.();
  };
  ...
  return (
    <div className="relay-profile-editor">
      <div className="relay-editor-head">
        ...
        {isNew ? null : (
          <Button
            disabled={!form.relayProfilesEnabled || actions.relaySwitching || Boolean(modelRowsError)}
            onClick={onSwitch}
            title={!form.relayProfilesEnabled ? t("供应商配置总开关已关闭") : actions.relaySwitching ? t("供应商切换中") : modelRowsError ?? undefined}
